...
subsequence that is strictly increasing. A subsequence is a
...
Output:
The output should be an integer representing the length of the
...
problem = tg.Variable(
    problem_text, requires_grad=False, role_description="the coding problem"
)
...
loss_system_prompt = """You are a smart language model that evaluates
code snippets. You do not solve problems or propose new code snippets,
only evaluate existing solutions critically and give very concise feedback."""
...
Does the code solve the problem? What is the runtime complexity?"""
